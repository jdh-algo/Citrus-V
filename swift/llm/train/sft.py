...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def _prepare_model_tokenizer(self, ):
        ...
        # print(f"===== get model processor")
        self.model, self.processor = args.get_model_processor()
        # print(f"===== model.mllm.lm_head.weight.shape: {self.model.mllm.lm_head.weight.shape}")
        # print(f"===== processor: {self.processor}, {self.processor.image_processor}")
        # print(f"======= vocab size: len(self.processor.tokenizer): {len(self.processor.tokenizer)}")
        ...
        # print(f"====processor in _prepare_model_tokenizer: {self.processor.image_processor}, do_resize: {self.processor.image_processor.do_resize}, size: {self.processor.image_processor.size}")

        self._prepare_generation_config()

    def _prepare_template(self) -> None:
        template = self.args.get_template(self.processor)
        # print(f"===== train.sft template: {template}")
    ...
    def run(self):
        ...
        # print(f"===== train_dataset sft: {train_dataset[0]}")

        train_dataset, val_dataset = self._encode_dataset(train_dataset, val_dataset)
        # print(f"===== train_dataset sft encoded : {train_dataset[0]}")
    ...
    def _encode_dataset(self, train_dataset, val_dataset):
        ...
        # print("===== train_dataset[0]: ", train_dataset[0]) # 这里还是原始数据

        if not is_grpo:
            if args.packing:  # default False
                ...
                train_dataset = preprocessor(
                    ...
                    load_from_cache_file=args.load_from_cache_file,
                    strict=args.strict)
                # print(f"===== train_dataset[0] templated: {train_dataset[0]}")
