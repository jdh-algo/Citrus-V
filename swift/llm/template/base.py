...
class Template(ProcessorMixin):
    ...
    def __init__(
        ...
        *,
        ...
        # print(f"====processor in template init: {processor.image_processor}, do_resize: {processor.image_processor.do_resize}, size: {processor.image_processor.size}")
    ...
    def _preprocess_inputs(
        ...
        # print(f"----- self.model_meta.is_multimodal: {self.model_meta.is_multimodal}") # True
        if self.model_meta.is_multimodal:
        ...
        # print(f"----- self.max_pixels: {self.max_pixels}") # None
        ...
        # print(f"------ load_images_origin: {load_images_origin}") # True
        ...
        # print(f"------ inputs.is_multimodal: {inputs.is_multimodal}") # True
    ...
    def _replace_start_image_tags(inputs: StdTemplateInputs):
        """
        这个函数用于处理输入消息中的特殊标签 '<start-image>'。

        功能说明：
        1. 检查用户最后一条消息是否以 '<start-image>' 标签结尾
        2. 如果存在该标签，则：
           - 将 generate_mode 设置为 True，表示需要生成图像
           - 从消息内容中移除 '<start-image>' 标签
        3. 将 generate_mode 标志设置到输入对象中
        """
        # compat
        generate_mode = False
    ...
    def encode(self,
        ...
        """
        # print("===== inputs in Template.encode: ori inputs", inputs)
        ...
        if isinstance(inputs, (InferRequest, TemplateInputs)):
            inputs = asdict(inputs)  # 这一步没变化, 本身就是dict
        # print("===== inputs in Template.encode: after asdict", inputs)
        # print(f"isinstance(inputs, dict): {isinstance(inputs, dict)}") # True
        extra_kwargs = {}
        if isinstance(inputs, dict):
            inputs = deepcopy(inputs)
            if not self.is_training:
                InferRequest.remove_response(inputs['messages'])
            inputs, extra_kwargs = StdTemplateInputs.from_dict(inputs)  # 这一步变成标准输入格式
            # print(f"===== inputs in Template.encode: after StdTemplateInputs.from_dict", inputs)
        elif isinstance(inputs, StdTemplateInputs):
            inputs = deepcopy(inputs)
        assert isinstance(inputs, StdTemplateInputs)

        # print("===== inputs in Template.encode:", inputs)
        self._preprocess_inputs(inputs)  # 这一步把输入图像加载进来了
        # print("===== inputs in Template.encode: preprocessed", inputs)

        # print("===== self.mode in Template.encode: ", self.mode) # train
    ...
    def _simplify_context_list(self, context_list: List[Context], loss_scale_list: List[float],
        ...
        if len(temp) > 0:
            res.append(''.join(temp))
            res_loss_scale.append(temp_loss_scale)
        # print(f"====== res in _simplify_context_list: {res}")
    ...
    def replace_bbox(self, bbox: List[int], index: int, inputs: StdTemplateInputs) -> List[Context]:
        ...
        Returns:
            The contents or input_ids replaced
        """
        # print(f"========== replace_bbox in base: {bbox}")
    ...
    def _pre_tokenize(self, context_list: List[Context], loss_scale_list: List[float],
        ...
        for context, loss_scale in zip(context_list, loss_scale_list):
            ...
            res += c_list
            res_loss_scale += [loss_scale] * len(c_list)
        # print(f"====== inputs in _pre_tokenize: {inputs}, context_list: {context_list}, res: {res}")
        # print(f"====== res in _pret: {res}")
        return res, res_loss_scale
    ...
    def _encode_truncated(self, inputs):
        # print(f"===== _encode_truncated: inputs: {inputs}")
        ...
        # print(f"===== _encode_truncated: inputs encoded: {encoded}")
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        """
        将输入的模板化数据编码为模型可处理的格式。

        此函数是模板系统的核心编码方法，负责将结构化输入转换为模型训练或推理所需的张量。
        主要功能包括:

        1. 确定使用的模板后端(swift或jinja)
        2. 根据后端类型调用相应的编码方法获取上下文列表和损失缩放列表
        3. 处理编码器-解码器模型或GKD模式下的特殊编码需求
        4. 简化上下文列表并将其转换为输入ID、标签和损失缩放因子
        5. 添加动态EOS标记
        6. 处理Megatron并行计算的特殊需求
        7. 在非训练模式下移除不必要的标签信息

        参数:
            inputs: 标准化的模板输入，包含对话历史、图像等多模态数据

        返回:
            包含编码后数据的字典，主要包括input_ids、labels和loss_scale等键值对
        """
    ...
    def _data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        ...
        """
        # print(f"====== batch[0] in base._data_collator: {batch[0]}")
    ...
    def _data_collator_mm_data(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(pixel_values_videos) > 0:
            ...
            # 立即释放原始张量列表，因为已经拼接完成
            for tensor in pixel_values_videos:
                del tensor
