...
#         image_resized = image.resize((self.target_size, self.target_size))
#         image_array = np.array(image_resized).astype(np.float32)

#         return torch.from_numpy(image_array).permute(2, 0, 1).contiguous().unsqueeze(0)
...
class DirectResize:
    ...
    def __call__(self, image: Image.Image) -> torch.Tensor:
        ...
        # 使用高质量的resize方法
        image_resized = image.resize((self.target_size, self.target_size), Image.Resampling.LANCZOS)

        # 转换为numpy数组，确保数据类型正确
        image_array = np.array(image_resized, dtype=np.float32)

        # 归一化处理
        image_array = image_array / 255.0  # 0-1范围
        image_array = (image_array - self.img_mean) / self.img_std

        # 使用numpy进行维度转换，避免PyTorch的permute操作
        image_array_transposed = np.transpose(image_array, (2, 0, 1))

        # 直接创建tensor
        tensor = torch.from_numpy(image_array_transposed.astype(np.float32))
        return tensor.unsqueeze(0)


class DirectResize_maks:

    def __init__(self, target_size: int) -> None:
        self.target_size = target_size

    def __call__(self, mask: Image.Image) -> Any:
        mask = mask.convert('L')
        mask_resized = mask.resize((self.target_size, self.target_size), resample=Image.NEAREST)
