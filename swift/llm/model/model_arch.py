...
class MultiModelKeys(ModelKeys):
    ...
    grounding_encoder: Union[str, List[str]] = field(default_factory=list)
    seg_projector: Union[str, List[str]] = field(default_factory=list)
    ...
    def __post_init__(self):
        ...
        if self.grounding_encoder is not None:
            v = getattr(self, 'grounding_encoder')
            if isinstance(v, str):
                setattr(self, 'grounding_encoder', [v])
            if v is None:
                setattr(self, 'grounding_encoder', [])

        if self.seg_projector is not None:
            v = getattr(self, 'seg_projector')
            if isinstance(v, str):
                setattr(self, 'seg_projector', [v])
            if v is None:
                setattr(self, 'seg_projector', [])
...
register_model_arch(
    MultiModelKeys(
        MLLMModelArch.citrus_v,
        language_model='mllm.model.language_model',
        aligner='mllm.model.visual.merger',
        vision_tower='mllm.model.visual',
