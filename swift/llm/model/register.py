...
def get_model_tokenizer_with_flash_attn(model_dir: str,
    ...
    # print(f"====== model_config 0: {model_config}")
    if model_config is None:
        model_config = AutoConfig.from_pretrained(model_dir, trust_remote_code=True)
    # print(f"====== model_config 1: {model_config}")

    AttnImpl.update_attn_impl(model_config, kwargs.get('attn_impl'), kwargs.get('attn_impl_keys'))
    kwargs['model_config'] = model_config
    # print(f"====== model_config 2: {model_config}")
...
def get_model_tokenizer_multimodal(model_dir: str, *args, **kwargs):
    ...
    # print(f"======== processor cdir: {model_dir}")
    processor = AutoProcessor.from_pretrained(model_dir, trust_remote_code=True)
    kwargs['tokenizer'] = processor.tokenizer
    # print(f"====== processor in get_model_tokenizer_multimodal: {processor}, model_dir: {model_dir}, size: {processor.image_processor.size}")
    model, _ = get_model_tokenizer_with_flash_attn(model_dir, *args, **kwargs)
    # print(f"************* model.mllm.lm_head.weight.shape: {model.mllm.lm_head.weight.shape}")
    return model, processor
...
def get_matched_model_meta(model_id_or_path: str) -> Optional[ModelMeta]:
    ...
    # print(f"===== model_name: {model_name}")
    # print(f"===== MODEL_MAPPING: {MODEL_MAPPING.keys()}")
...
def get_model_tokenizer(
    ...
    # print(f"===== model_dir: {model_dir}, model_info: {model_info}, model_kwargs: {model_kwargs}")
