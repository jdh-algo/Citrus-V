...
class PtEngine(InferEngine):
    ...
    def _infer_full(self,
        ...
        # 提取所有自定义字段，排除常规字段
        custom_fields = {k: v for k, v in output.items() if k not in ['sequences', 'logits', 'past_key_values']}
            ...
            for j in range(num_return_sequences):
                ...
                choices.append(
                    ChatCompletionResponseChoice(
                        ...
                        finish_reason=finish_reason,
                        logprobs=logprobs))
            # 将custom_fields透传到raw_response
            res.append(
                ChatCompletionResponse(
                    model=self.model_name, choices=choices, usage=usage_info, raw_response=custom_fields))
        return res
