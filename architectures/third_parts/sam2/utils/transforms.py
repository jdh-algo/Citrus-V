...
class SAM2Transforms(nn.Module):
    ...
    def transform_boxes(self, boxes: torch.Tensor, normalize=False, orig_hw=None) -> torch.Tensor:
        ...
        if the coords are in absolute image coordinates, normalize should be set to True and original image size is required.
        """
        boxes = self.transform_coords(boxes.reshape(-1, 2, 2).contiguous(), normalize, orig_hw)
        return boxes
    ...
    def postprocess_masks(self, masks: torch.Tensor, orig_hw) -> torch.Tensor:
        ...
        if self.max_hole_area > 0:
            ...
            mask_flat = masks.flatten(0, 1).unsqueeze(1).contiguous()  # flatten as 1-channel image
            labels, areas = get_connected_components(mask_flat <= self.mask_threshold)
            is_hole = (labels > 0) & (areas <= self.max_hole_area)
            is_hole = is_hole.reshape_as(masks).contiguous()
