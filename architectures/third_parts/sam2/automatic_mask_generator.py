...
from .utils.amg import (MaskData, area_from_rle, batch_iterator, batched_mask_to_box, box_xyxy_to_xywh,
                        build_all_layer_point_grids, calculate_stability_score, coco_encode_rle, generate_crop_boxes,
                        is_box_near_crop_edge, mask_to_rle_pytorch, remove_small_regions, rle_to_mask,
...
class SAM2AutomaticMaskGenerator:
    ...
    def _process_batch(
        ...
        data = MaskData(
            masks=masks.flatten(0, 1).contiguous(),
            iou_preds=iou_preds.flatten(0, 1).contiguous(),
            points=points.repeat_interleave(masks.shape[1], dim=0),
            low_res_masks=low_res_masks.flatten(0, 1).contiguous(),
        )
        del masks
